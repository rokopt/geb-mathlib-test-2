...
check_file() {
    ...
    if OUTPUT=$(lake env lean "$FILE" 2>&1); then
        ...
        while IFS= read -r line; do
            # Match new Lean output format (v4.30.0-rc2):
            ...
            if [[ "$line" =~ ^\'?([a-zA-Z0-9_.]+)\'?[[:space:]]+depends[[:space:]]+on[[:space:]]+axioms:[[:space:]]*(\[([^]]*)\])? ]]; then
                ...
                # New format: parse axioms inline from [list]
            ...
            # Old multi-line format: axiom name on its own line after
            # the depends-on header. Kept for compatibility with older
            # Lean versions that emit one axiom per line.
